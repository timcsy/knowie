...
- **實際發生**：兩條被反覆引用、卻從沒寫進真相源——「多條獨立推導＝真根」（引用 4 次才發現不在 [收斂](concepts/發散的自我維護是在自動製造熵.md)）、「可見性錯覺」（引用整個 session 才發現不在 experience）。**用熟了 → 以為固化了**，但它只活在「程序記憶」（怎麼做事），沒升進「語意記憶」（可引用的概念/教訓，見 [記憶系統](concepts/記憶系統.md)）。諷刺：兩條都是「怎麼維護知識庫」的 meta 判準——越貼身用，越忘了它沒被寫下。
...
- **教訓**：要加 CLI 前先問「**AI 現在在場嗎？在場就讓它做**」。這是我反覆犯、該被擋下的觸發（見 [收斂](concepts/發散的自我維護是在自動製造熵.md) 失敗模式）。
...
- **驗證（2026-08-17）·oracle 賭注有第一個樣本了，而失敗模式是「過期的正面宣稱」**：code-battle 放置三個半月後跑 migrate + judge。judge 的「對照現況程式碼」那一步**實際去執行了測試**，得到 `587 failed | 122 passed`——而 vision 上寫著「自動測試 703/736 通過」，那個數字來自四個半月前的一個 commit，之後沒有人再量過。根因是 `db.ts` 的 migration 順序讓 `createTestDb` 拋錯，61/69 個測試檔跑不起來：**測試存在但不能跑，等於回到零測試**。judge 抓到後三個 commit 把 587 降到 17，並揪出三個真實程式錯誤。
  - 所以「未驗賭注」要改口：**它在「夠老、why 真的爛掉」的專案上成立**——而那個條件過去不出現，只因為 next 一直在預防、專案沒被放置過。
  - 更該記的是失敗模式：**爛掉的 why 最危險的形態不是缺漏，是一個過期的正面宣稱**（「測試 703/736 通過」）。缺漏會讓人去問，正面宣稱讓人不問。抓到它的不是讀那句話，是 [`_core`](../skills/_core.md) 那條不變量「宣稱讀過就附逐字證據」——judge 去**跑**了測試。
  - **不適用的時候**：這條不證明 judge 能抓所有腐爛的 why。它抓到的是**有 oracle 可跑**的那種（測試、build、可量的數字）；純判斷型的 why（某條原則還算不算對）依然沒有 oracle，仍屬未驗。
- **來源**：使用者十幾個專案真實使用回饋（2026-06-07）+ dogfood next 漏召回。現場 → [episode](episodes/2026-06-07-next規劃自己揭露召回盲點.md)。2026-08-17 的驗證證據在外部 repo：`code-battle` commit `e9a3301`（judge 修正失實記載）、`635974b`／`6922851`（後續修復）。
...
- **擴張（2026-08-14）·同一條判準沒被套到相鄰器官**：這條當初只寫給 skill。五專案外推審計顯示**它對 `experience` 同樣成立、而沒有人把它搬過去**——教訓寫成**做法**（「把綁定層與算繪分離就能測」）會隨技術棧過期（換掉那個框架就死），寫成**判準**（「一個會改變被測對象的量測器，量到的不是被測對象」）才跨處境。判準式的極限形態是 semorphe 那條：查完六十年文獻後，它**改掉的不是結論，是驗收的寫法**（「訊息變好了」→「兩個數字都要記，而假警報只准下降」）。⇒ 驗收標準：**一條教訓完成的標誌，是它改掉了某個判準的寫法，不是它被記下來了。**（這也是「反覆使用的隱性判準會以為已固化」的變體——那條是用熟了以為記了，這條是**寫下了、卻沒套到隔壁器官**。）轉移見 [history/013](history/013-概念命名從名詞式到斷言式.md)
...
### 「可機械化」不是想出來的，是拿真實分佈驗出來的——同一批提案我錯了兩次

- **理論說**：一條檢查該不該機械化，看它的定義清不清楚就知道了。定義得出來 → 寫成 grep。
- **實際發生**：五專案審計後我提了六條「可 grep 的檢查」，跑六個真實庫，**兩條當場垮掉，而且垮的方式不同**：
  - `history/` 檔名比對轉移詞 → **五個庫誤報**。`型別統一`／`改用`／`降級為`／`固化`／`先於` 全是真轉移。定義很清楚，但「轉移」的詞彙是**開放集**。
  - 粗體密度 → 我先用「所有 `**…**` / 行」，把每條教訓固定的段落標籤算成強調，**等於在懲罰格式合規**；knowie 自己因此被我判成「全集最差 1.29」。改成只數行內之後是 0.61 的中段。而**修正後六庫落在 0.42–0.77、我主觀認為最好的那庫在 0.45**——這個量根本分不開好壞。
  - 對照組：另四條（舊欄位寫成進度／勾選比／整層孤兒率／缺來源）**零誤報**，每一條紅都是人工確認過的真病灶。
- **解決方式**：兩條各退到它該在的層——檔名那條退成 `history-README` 的**命名提示**（對作者是好判準，對掃描器是壞規則）；強調那條退成 `_core` 的**寫作判準**，不進 judge。留下的四條才是規則。
- **教訓**：一條檢查是「第一型（可定義的範疇錯）還是第二型（判斷變異）」，**不是從定義看得出來的，要拿真實分佈跑**。而失敗有兩種面貌：**詞彙開放**（規則列不完）和**分不開**（有數字但健康與否落在同一區間）。兩種都不是「規則寫得不夠好」，是那條根本不該是規則。
- **不適用的時候**：手上沒有真實語料時不成立——這次能分得開，是因為驗收資料是**五個既有的真實庫**，不是我自己編的例子。若只有一兩個樣本，看起來會像規則有效。
- **相關**：「[把 migrate 當冪等函數修是用錯標準]」的第一型／第二型是同一把刀（那條講**產出**該不該收斂，這條講**檢查**該不該是規則）；也是「[有些 bug 是執行層的，協議層解不了]」的鄰居——那條說「別預設用措辭修」，這條說「也別預設改得成掃描」。
- **來源**：0.7.3 批次一建完跑六庫 fixture（2026-08-14）。被否決的兩條見 [history/012](history/012-否決兩條機械檢查-檔名詞彙與強調密度.md)；現場 → [episode](episodes/2026-08-14-五專案外推審計.md)。
...
### 規則只寫在散文裡不算落實——沒有可數檢查的條款，會被自己的掃描器判為健康
- **理論說**：規則已經寫進 skill 了（judge §4「retire the roadmap item and its source design draft」、experience 這條「draft 留到完成才退場」），照著做即可；漏做是執行者的問題，措辭沒錯。
- **實際發生**：KnowField 累到 **26 份「其 roadmap 項已完成、卻還在 `draft/`」**，104 個引用點指著它們。judge 跑幾次都全綠——因為 §3 的機械檢查只有「Draft↔vision sync」，而它查的是「已晉升的 draft 有沒有帶標記」。一份被 `[x]` 項連著、又掛著 in-flight 標記的 draft，**同時滿足「有標記＝健康」和 §5「被 roadmap 連到的 draft 永不收割」**——兩條檢查聯手把債認證成健康狀態。
- **解決方式**：judge §3 新增 **退場債**檢查：`grep` vision 裡還連著 `draft/` 的 `[x]` 項，健康值是 **0**（絕對數，不是比率——退場是事件觸發的，不像 done:open 是流量比）；報成一個 🔴，旁邊附 inbound 數當改指工作量。§4 補操作順序：**先抽、再改指、最後刪**。
- **教訓**：judge §3 的立論（「Run these with grep/ls — by eye, both humans and AIs miss them」）**適用於 skill 自己的條款**。一條規則若只活在散文段落、沒有對應的可數檢查，它的落實率就是機率性的；而且會敗得無聲——不是報錯，是全綠。**判準：一條新規則寫完，問「它被違反 26 次的時候，哪個 grep 會亮？」答不出來，這條就還沒落實。**
- **不適用的時候**：不是每條都要配檢查。配得起檢查的是**有可數犯罪現場**的（檔案還在、連結還在、數字對得上）；純判斷型的（這條原則還算不算對）配不了，那種靠人——同 [`history/012`](history/012-否決兩條機械檢查-檔名詞彙與強調密度.md) 否決兩條機械檢查的分界。
- **已落實**：[`skills/knowie-judge/SKILL.md`](../skills/knowie-judge/SKILL.md) §3「Retirement debt」＋§4 順序條款。
- **來源**：[episodes/2026-08-26-KnowField退場債現場](episodes/2026-08-26-KnowField退場債現場.md)、[`history/014`](history/014-退場債-規則入散文不算落實.md)。

### 當正確的操作會讓健康指標暫時變差，指標就會被拿來否決那個操作
- **理論說**：健康檢查是中立的偵測器——它報告狀態，不干涉決策。多加一條檢查只會讓基地更健康。
- **實際發生**：使用者問「那 26 份 draft 該不該退場」，我算完之後回答「刪掉會產生 31 條死連結」——**用 judge 自己的紅燈去反對 judge 自己的退場規則**。死連結是 §3 明列的 rot，而退場必然製造死連結，所以「提議刪除」在基地自己的計分板上讀起來就是「提議製造損壞」。使用者一句話戳破：**「死連結不是就是要修正的事嗎？不然 draft 要放在那裡多久？」**
- **解決方式**：在指標旁邊標明它與哪個正確操作衝突，並指定該數字的**正確讀法**——judge §3 死連結那條補上：待退場所**將要**造成的死連結不是 rot，是**改指工作清單**，引用它時只能當步驟二的長度，不能當保留檔案的理由。
- **教訓**：這是 [讓認錯變便宜](concepts/讓認錯變便宜.md) 的規範投影再往外一格——不只「機制不該升高認錯成本」，是**任何機制都不該讓正確的操作在自己的指標上看起來像損壞**。否則指標就從偵測器變成了合理化的彈藥庫，而且是最好用的那種：它有數字、有紅燈、聽起來像在維護品質。**判準：這條檢查會不會在某個正確操作進行到一半時亮紅燈？會的話，就必須寫明那個豁免——否則它遲早被當否決票用。**
- **已落實**：[`skills/knowie-judge/SKILL.md`](../skills/knowie-judge/SKILL.md) §3 Dead links 那條的豁免句；概念投影寫進 [讓認錯變便宜](concepts/讓認錯變便宜.md)。
- **來源**：[episodes/2026-08-26-KnowField退場債現場](episodes/2026-08-26-KnowField退場債現場.md)。

## 關鍵延伸（主題觸發必讀）

| 觸發關鍵字 | MUST 讀 |
|---|---|
| 命名 / 捕捉非發明 / 私語 / 批次結晶 | [concepts/蒸餾](concepts/提煉是每一層都在做的同一件事.md) |
...
| 退場債 / draft 不退場 / 規則沒有檢查 / 指標反對正確操作 / 死連結當否決票 | [history/014](history/014-退場債-規則入散文不算落實.md) + [episodes/2026-08-26-KnowField退場債現場](episodes/2026-08-26-KnowField退場債現場.md) |
| 教訓的形狀 / 判準式 vs 做法式 / 四段式 / 來源那條腿 / 強調預算 | [episodes/2026-08-14-五專案外推審計](episodes/2026-08-14-五專案外推審計.md) |
