...
**狀態/觸發**：**未驗、大重建**。不是現在建——我們剛決議「good-enough + HITL」（見 [history/009](../history/009-migrate運作模型從snapshot判斷到時間軸replay.md)）。**觸發＝當 good-enough/HITL 撐不住、或要正面解收斂時，這就是該重寫的樣子**；且因它溶掉補丁，重寫即 distill（呼應 experience「skill 該寫判準非窮舉」+ 爵士 Shell Chord 嚮往）。
