...
## 下一代架構：orchestrator + 平行 sub-agents + merge + 反芻收斂（2026-06-13，未驗）⭐
現行＝sequential-masked-fold + 十幾條補丁。這是一個**另一個生成架構**，可能同時解掉「平行（快）／結構-why 分離／收斂」三件——是架構級的 Shell Chord，不是又一個補丁。

**AR vs Diffusion 的根**：開發是 **autoregressive**（真的 forward、不知未來）；migrate 是考古、手上有整份記錄，**不必假裝 AR**。masked-AR 換來「當時 why」忠實，但付出**全局結構不穩**（slice/分組 run-to-run 飄＝收斂問題）。**diffusion**（從粗到精反覆去噪、會收斂到資料流形）擅長全局一致。→ migrate 想要兩個：結構的全局一致（diffusion）+ why 的因果忠實（masked-AR）。

**分工（結構極 vs why 極，資訊權限相反）**：
- **orchestrator（大 context ＝ 結構極，diffusion 式、會收斂）**：決定怎麼切（錨在 git first-parent 候選邊界上做全局分組）、平行派發、**merge 時偵測跨片 transition**、拼結構、墓碑/projection-edit 等範疇規則在**這一處**全局套用。
- **平行 sub-agents（why 極，遮罩、局部）**：每片只做**局部抽取**＋當時 stated why。**可平行**，因為局部、每片 spec/commit 自包含。
- **兩種 why 不同權限**：「當初為何選 X」遮罩（sub-agent 不看後面）；「為何後來改了」由 orchestrator 重建（**合法**看兩端，transition 本就跨在兩端之間）。

**為何平行這次安全（0.6.7 精確化、非推翻）**：當初禁平行的真因是「sub-agent 自己要偵測 transition 卻沒累積 base」。把 **transition 上移給 orchestrator** → sub-agent 不再需要 base → 平行安全。教訓改寫成：**做 transition 偵測的要 sequential-with-base；把它上移，做局部抽取的就能平行。**

**反芻＝收斂引擎，但前提是對固定文本**：「聖經一讀再讀」會收斂，是因為**文本固定**。所以反芻紀律＝**每輪 re-ground 回 git 證據（固定文本），不是對自己上一版潤稿**（後者 telephone-drift）。git 是那本聖經；反芻一讀再讀它、不是讀自己的讀後感。HITL ＝ 反芻中的一個去噪步（guided diffusion）。

**三條讓它成立的紀律**（否則重蹈 0.6.5/鐘擺）：
1. 平行只做局部；transition 一律 orchestrator merge 時做。
2. orchestrator 全局視野**只產結構**（切片/轉移/墓碑/拼法），**不重寫局部 why**。
3. orchestrator 切片**錨在 git 結構**（first-parent 給候選），只做全局分組——不自由心證切（否則非決定性從切片溜回）。

**它本身就是一次 distill——哪些補丁會溶解**：
- 「sequential carrying base」→ 溶解（平行 + orchestrator 管 transition）。
- 「跨片 transition＝比對累積 history」→ 變 orchestrator merge 本職（有全局、更乾淨）。
- 「projection-edit ≠ event」「墓碑 ≠ 轉移」→ 仍要，但由 orchestrator 在**一處**全局套，不必每個 sub-agent 各記。
- **收斂**（最大未解）→ 被反芻解掉（diffusion 收斂 + 對固定 git）。
- 同構：map-reduce-refine、整合性 agent、歷史學家實際工法（先掃全貌定章節→平行寫→反覆對原始史料重讀整稿）。

**狀態/觸發**：**已建（0.7.0，2026-06-13）、核心賭注已 A/B 驗（battle，見下方 ✅）、廣度未驗**。使用者選擇「先做出來」→ migrate skill 已重寫成 encoder-decoder 四相（A 結構/B 平行遮罩 why/C merge/D 反芻），補丁溶進各 phase（skill 98→85 行＝distill 兌現）。它部分軟化 [history/009](../history/009-migrate運作模型從snapshot判斷到時間軸replay.md) 的「認下非冪等」——**結構這層其實能收斂**（encoder 一次算 + 反芻對固定 git），why 那層仍是抽樣；009 待廣度驗後補註。

### 最利的鏡子：RNN → Transformer（這想法的來源）
- **現行 fold ＝ RNN**：左到右、累積 base 當 **hidden state** 往前帶。**你提的架構 ＝ Transformer**：平行 + 靠 attention 直接連任意位置（orchestrator 大 context ＝全局 attention）。
...
- ✅ **0.7.0 encoder-decoder 的中心賭注 A/B 驗了（battle，2026-06-13）**：新架構在 battle 重跑——`history/002 作品卡互動結構` **接到了長程 transition**：M1 把整張卡做成 `<button>`（純文字時合理）→ 很多片之後 feature 006/007 塞進 iframe/媒體才回溯塌成 P0 破綻；還把兩個看似無關的故事（US1 投票破綻 + US6 a11y）綜合成同一根因。**這正是 sequential-fold（RNN）會把早決策稀釋進累積 base 而漏掉的長程依賴，encoder 全局 attention 接住了** → 中心賭注成立。附帶驗：墓碑分組成一張目錄（0.6.13 修生效、非 0.6.12 的散落偽轉移）；concepts 10→2 母概念（distill 真發生，如「真實環境邊界」收斂一堆具體 bug）；日期全 git 當時、無 06-13；無 projection-edit regression。
- ⚠️ **0.7.0 的 caveat（誠實）**：只**一專案一跑**；**收斂仍沒乾淨量**（同版連跑沒做，仍是 optional）；發現鐘擺盪到另一邊——`帳號→金鑰`（principle 5 被改）被 **over-tombstone**（降級成墓碑），真根是 tombstone≠transition 的「in force」沒涵蓋 stated principle/plan（Type-1，待補一句規則，低優先）。
- 🔨 **待建（執行層，非 skill 措辭）**：
