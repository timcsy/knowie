...
- ⏳ **收斂性**：要二跑比對才證——但八成要等遮罩 harness 才會真穩（偷看 = 每跑合理化角度不同 = 不收斂）。
- 設計脈絡同族：[記憶系統](../concepts/記憶系統.md)（git＝原始因果基底）、[收斂](../concepts/收斂.md)（冪等核心、現行 migrate 違反它）。
- 待想：切片邊界怎麼自動抓（里程碑 tag？commit pattern？spec 目錄？）。
