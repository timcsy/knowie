...
**狀態/觸發**：**已建（0.7.0，2026-06-13）、未驗**。使用者選擇「先做出來」→ migrate skill 已重寫成 encoder-decoder 四相（A 結構/B 平行遮罩 why/C merge/D 反芻），補丁溶進各 phase（skill 98→85 行＝distill 兌現）。**待 battle A/B**（對舊 sequential-fold 比），驗過才取代。它部分軟化 [history/009](../history/009-migrate運作模型從snapshot判斷到時間軸replay.md) 的「認下非冪等」——**結構這層其實能收斂**（encoder 一次算 + 反芻對固定 git），why 那層仍是抽樣；009 待驗後補註。
