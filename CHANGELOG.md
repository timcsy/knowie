...
- **`_core`: emphasis is a budget.** Bold marks the criterion itself, ⚠️ marks "I got this wrong right here", everything else stays plain. Measured bold density across the audited bases ran 0.37–0.81 per line, and the single best-written concept file carried 20+ ⚠️ on one page — at which point the genuinely load-bearing sentences sink. This costs more with an AI reader than a human one: emphasis is a signal aimed at attention, and diluting it lowers recall precision, which is the mechanism `/knowie-next` actually runs on.
