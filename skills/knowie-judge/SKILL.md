...
- **Link notation first — an orphan rate is meaningless until the pointers are greppable**: before reporting any graph number, measure how the base actually points at files. Count as non-compliant: a `[[wikilink]]` **with content** (an empty `[[ ]]` is prose *about* the notation, not a link), and a backtick `` `path.md` `` that **resolves to a file that exists in this base** and isn't a generic name (`SKILL.md`, `README.md`, the three core files) — those are illustrations, not pointers. Derived principle 8 mandates `[]()` precisely so the graph can be *derived*; the other two forms are edges an AI can follow by name but **grep cannot see**, so every downstream number (orphans, backlinks, dead links) comes out wrong, and wrong in the direction that reads as catastrophic. Past **~10%** non-compliant, report the notation as *the* finding ("N pointers aren't `[]()` — converting them is find-and-replace") and label the orphan rate "pending notation fix". Sending someone to reorganize a knowledge base when the real problem is link syntax is the most expensive kind of false positive.
  - Measured across seven real bases: 0.0% / 1.4% / 2.3% / 2.5% then a gap to 11.8% / 13.4% / 22.4% — healthy and indebted separate cleanly, which is why this one earns a threshold where emphasis density didn't (see `history/012`).
