...
如果你還沒裝 SpecKit，在終端機執行：

```bash
npx spec-kit init
```

然後再跑一次 Knowie 的更新，讓它偵測到新裝的工具並注入引用：

```bash
npx knowie update
```

Knowie 會在 SpecKit 的 `.specify/memory/constitution.md` 裡注入你的 `knowledge/` 引用。這樣 SpecKit 在寫規格的時候，就能讀到你的原則、願景和經驗。
...
建議控制在每份 100-200 行以內。它們是「索引」，不是「百科全書」。詳細的研究放 `research/`，設計決策放 `design/`，事件紀錄放 `history/`。核心文件只放精煉的結論。
...
非常容易。刪掉 `knowledge/` 目錄，然後在 AI 工具的設定檔裡移除被 `<!-- knowie:start -->` 和 `<!-- knowie:end -->` 包起來的區塊就好。
