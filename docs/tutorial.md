...
你需要 **Node.js 18 以上**（`node -v` 確認）和**一個 AI 工具**（Claude Code 支援 slash commands、體驗最完整；GitHub Copilot、Gemini CLI 也可以）。

> **作者的一鍵捷徑（選配）：** 我自己用 [`npx prespec`](https://www.npmjs.com/package/prespec) 一次把環境裝好 —— Node、Git、常見 AI 助手（Claude Code、Copilot、Codex…）和 Spec Kit。不想用就照各工具官網手動裝即可。
...
我們的遊戲叫 **Runefall**（符文隕落）。

### （選配）先設定 SpecKit

如果你打算用 SDD（規格驅動開發），**先把 spec 工具設好、再裝 knowie** —— 這樣 knowie 會偵測到它，把你的 `knowledge/` 引用注入進去。

```bash
specify init . --ai claude --ai-skills
```

（`--ai` 換成你習慣的工具即可。）然後打開 AI 工具，跑一次 SpecKit 的憲章設定，例如：

```
/speckit.constitution 使用 TDD。請用繁體中文撰寫規格文件與回答。
```

**不用 SDD？整段跳過** —— knowie 本身獨立運作，不依賴任何 spec 工具。
...
最方便的方式 —— 直接把這行**貼進你的 AI 聊天視窗**（Claude Code、Codex 等 CLI 工具都行）：
...
> **想自己在終端機跑、逐項確認？** 拿掉 `--yes` 用 `npx knowie init`，它會列出偵測到的工具、問你 `Proceed? (Y/n)` 再進行。
...
> **SpecKit 已在「前置準備」設好了**（`specify init` + `/speckit.constitution`），而且因為你是**先設 SpecKit 再裝 knowie**，knowie 的 `npx knowie init` 已經把你的 `knowledge/` 引用注入到 `.specify/memory/constitution.md`——SpecKit 寫規格時就讀得到你的原則、願景、經驗。（若你是反過來、後加的 SpecKit，補跑一次 `npx knowie update` 讓它注入即可。）
...
到這裡你已經體驗了完整的流程。日常營運一個專案，循環就是「**規劃 → 做一輪 → 對齊**」，一圈一圈轉：

```
  /knowie-next ──→ 規劃這一輪要做什麼（基於原則／願景／經驗）
      │
      ▼
  跑一輪（任一種就好——knowie 不在意你用哪個）：
      • SpecKit：specify → plan → tasks → implement
      • Loop Engineering：跑一個 loop
      • 最樸素：plan mode → 實作完，算一輪
      │
      ▼
  /knowie-judge ──→ 對齊知識庫 + 健康檢查（踩到的坑、變了的方向都在這收回來）
      │
      ├── 冒出新想法？ 先 `chat only:` 聊一聊，覺得不錯 → /knowie-capture 存進 draft
      ├── draft 累積太多？ → /knowie-consolidate 固化到適當的地方
      │
      └──────────────→ 回到 /knowie-next，做下一輪
```

沒有新想法時，就是最單純的 `/knowie-next` → 做一輪 → `/knowie-judge` → `/knowie-next` → 做一輪 → `/knowie-judge`…… 這樣轉。
...
1. （前置）裝環境 + 選配設好 SpecKit（`specify init` / `/speckit.constitution`）
2. `npx knowie init --yes`（貼進 AI 聊天）+ `/knowie-init` — AI 引導你填知識文件（舊專案則 `/knowie-migrate` 從 git 重建）
3. **營運循環**：`/knowie-next`（規劃）→ 做一輪（SpecKit／loop／plan mode 任一）→ `/knowie-judge`（對齊）→ 回到 next
4. 有新想法時：`chat only:` 聊 → `/knowie-capture` 存進 draft；draft 多了 → `/knowie-consolidate` 固化

從「為什麼做」（Knowie）到「做什麼」（規格／loop）到「怎麼做」（Code），整條鏈都有你的知識在背後串著——而且你換哪個引擎都行。
